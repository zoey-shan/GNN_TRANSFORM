...
parser.add_argument('-t', '--train', default=64, type=int, help='train batch size, default 100')
...
parser.add_argument('--annotation', default='F:/Feature/MISAW_train/Procedural decription/annotation.txt', type=str, help='annotation path')
...
def get_data():
    ...
    phase_dict = {'Idle': 0, 'Needle holding': 1, 'Suture making': 2, 'Suture handling': 3, '1 knot': 4, '2 knot': 5, '3 knot': 6, 'Idle Step': 4}
    ...
    train_dataset = CholecDataset(image, train_image, train_phases, train_kdata, train_transforms)
    test_dataset = CholecDataset(image.replace('train','test'), test_image, test_phases, test_kdata, test_transforms)
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            if train_corrects.cpu().numpy() / train_num > 0.8:
                torch.save(copy.deepcopy(model.state_dict()), str(epoch)+'.pth')
