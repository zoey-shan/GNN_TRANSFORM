...
class CNN(nn.Module):
    def __init__(self, in_channels=3, mid_channels=6, kernel_size=49):
        super(CNN, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, mid_channels, kernel_size)
        self.conv2 = nn.Conv2d(mid_channels, 1, kernel_size)
...
class TcnTransformer(nn.Module):
    def __init__(self, class_num, k_num, hidden_state, encoder_params,
        ...
        self.lstm = nn.LSTM(hidden_state, k_num, 1, batch_first=True)
    ...
    def forward(self, x_vision, x_kinematics):
        # print('Checkpoint 1', x_vision.size())
        x_vision = self.cnn(x_vision)
        x_vision = self.tcn_vision(x_vision)
        # print('Checkpoint 2', x_vision.size())
        x_kinematics = torch.unsqueeze(x_kinematics, -1)
        x_kinematics = self.tcn_kinematic(x_kinematics)
        x_kinematics, _ = self.lstm(x_kinematics)

        img_node = x_vision.permute(1, 0, 2)
        # print('Checkpoint 3', img_node.size())
